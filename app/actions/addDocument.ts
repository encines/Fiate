...
export async function addDocument(prevState: any, formData: FormData) {
  ...
  try {
    ...
    for (const item of allItems) {
      ...
      if (item instanceof File && item.size > 0) {
        if (!item.type.startsWith("image/")) {
          return {
            error: `El archivo ${item.name} debe ser una imagen válida.`,
          };
        }
        if (item.size > 10 * 1024 * 1024) {
          return {
            error: `El archivo ${item.name} no debe superar los 10 MB.`,
          };
        }
    ...
    const { error: insertError } = await supabase.from("CarDocument").insert({
      userCarId,
      type,
      name,
