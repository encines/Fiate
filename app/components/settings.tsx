...
export default function Settings({ cars = [], car, user }: SettingsProps) {
  ...
  const updatePref = (key: string, value: string) => {
    ...
    if (key === "theme") {
      setTheme(
        value === "Sistema" ? "system" : value === "Oscuro" ? "dark" : "light",
      );
    } else {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 p-6 rounded-[24px] border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30">
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-sm text-zinc-500">
              Configura tu identidad pública en Fiate.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-500 text-sm mt-1">
              Gestiona los autos registrados en tu flota.
            </p>
          </div>
          <div className="hidden sm:flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-100 dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              ...
              strokeLinejoin="round"
            >
              <path d="m21 8-2 2-5-5 2-2" />
              <path d="M11 3H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
              <path d="M7 15h0" />
              <path d="M12 15h0" />
              <path d="M17 15h0" />
            </svg>
        ...
        <div className="grid gap-4">
          {cars.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-lg sm:text-xl text-zinc-900 dark:text-white truncate">
                      {car.brand} {car.model}
                    </p>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 text-xs sm:text-sm text-zinc-500 dark:text-zinc-500">
                      <span>{car.year}</span>
                      <span className="h-1 w-1 rounded-full bg-zinc-300 dark:bg-zinc-700"></span>
                      <span className="truncate">
                        {car.licensePlate || "Sin placas"}
                      </span>
                    </div>
                  </div>
                </div>
                <button
                  onClick={() =>
                    openDeleteModal(car.id, `${car.brand} ${car.model}`)
                  }
                  disabled={isPending}
                  className="flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 text-sm font-bold text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all disabled:opacity-50 active:scale-95 border border-rose-500/10 sm:border-0"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="18"
            ...
            <div className="flex flex-col items-center justify-center py-12 rounded-[24px] border border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/10">
              <p className="text-zinc-500 dark:text-zinc-500 font-medium px-4 text-center">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <section className="glass-panel p-6 sm:p-8 border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/40 rounded-[32px] backdrop-blur-md">
          ...
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800 rounded-[24px] border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm transition-colors">
            {/* Notificaciones */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white dark:bg-zinc-900/20 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors gap-2">
              ...
              <select
                ...
                className="appearance-none bg-transparent text-zinc-900 dark:text-white text-sm font-bold focus:outline-none cursor-pointer sm:text-right"
              >
                <option className="bg-white dark:bg-zinc-900" value="Activadas">
                  Activadas
                </option>
                <option
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white dark:bg-zinc-900/20 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors gap-2">
              ...
              <select
                ...
                className="appearance-none bg-transparent text-zinc-900 dark:text-white text-sm font-bold focus:outline-none cursor-pointer sm:text-right"
              >
                <option className="bg-white dark:bg-zinc-900" value="MXN">
                  MXN
                </option>
                <option className="bg-white dark:bg-zinc-900" value="USD">
                  USD
                </option>
                <option className="bg-white dark:bg-zinc-900" value="EUR">
                  EUR
                </option>
              </select>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white dark:bg-zinc-900/20 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors gap-2">
              ...
              <select
                ...
                className="appearance-none bg-transparent text-zinc-900 dark:text-white text-sm font-bold focus:outline-none cursor-pointer sm:text-right"
              >
                <option className="bg-white dark:bg-zinc-900" value="Oscuro">
                  Oscuro
                </option>
                <option className="bg-white dark:bg-zinc-900" value="Claro">
                  Claro
                </option>
                <option className="bg-white dark:bg-zinc-900" value="Sistema">
                  Sistema
                </option>
              </select>
            </div>
        ...
        <section className="glass-panel p-8 border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/40 rounded-[32px] backdrop-blur-md">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-8 leading-relaxed">
            Mantén tu cuenta protegida cambiando tu contraseña periódicamente y
            cerrando sesiones activas en otros dispositivos.
          </p>
          ...
          >
            <div className="relative w-full max-w-md rounded-[32px] border border-zinc-200 dark:border-zinc-700/50 bg-white dark:bg-zinc-900 p-8 shadow-2xl animate-in fade-in zoom-in duration-300">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 leading-relaxed">
                ...
                de tu garaje. Esta acción es permanente y se borrará todo su
