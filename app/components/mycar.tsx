...
export default async function MyCar({ car }: MyCarProps) {
  ...
  const quickStatus = [
    { label: "Kilometraje", value: `${currentKm.toLocaleString()} km`, tone: "text-indigo-500 dark:text-indigo-300" },
    { label: "Último servicio", value: lastService ? formatDate(lastService.date) : "N/A", tone: "text-teal-500 dark:text-teal-300" },
    { label: "Seguro", value: "Vigente", tone: "text-teal-500 dark:text-teal-300" }, // Mock
    { label: "Verificación", value: "Pendiente", tone: "text-violet-500 dark:text-violet-300" }, // Mock
  ];
        ...
        <div className="glass-panel rounded-[28px] p-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-4">
              <p className="text-sm uppercase tracking-[0.22em] text-zinc-500 dark:text-zinc-400">Mi auto</p>
              <div>
                <h1 className="text-3xl font-semibold sm:text-4xl">
                  {catalogCar.model.brand.name} {catalogCar.model.name}
                </h1>
                <p className="mt-2 text-zinc-500 dark:text-zinc-400">Información general y estado actual del vehículo.</p>
              </div>
              <EditVehicleModal userCar={car} />
            </div>

            <div className="relative aspect-[2.3/1] w-full max-w-md overflow-hidden rounded-[28px] border border-zinc-200 dark:border-zinc-700/80 bg-zinc-50/60 dark:bg-zinc-950/60 p-4 transition-colors">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {specs.map((item) => (
                <div key={item.label} className="soft-card rounded-2xl px-4 py-3">
                  ...
                  <p className="mt-2 text-sm font-medium text-zinc-900 dark:text-zinc-100">{item.value}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="mt-4 space-y-3">
              {nextTasks.map((task: any) => (
                <div key={task.title} className="soft-card rounded-2xl p-4">
                  ...
                  <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{task.detail}</p>
                </div>
              ))}
            </div>
          </div>
