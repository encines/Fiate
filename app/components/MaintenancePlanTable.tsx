...
export default function MaintenancePlanTable({ tasks, history, currentKm, userCarId, userPlan }: MaintenancePlanTableProps) {
  ...
  const handleAIDiscovery = () => {
    setConfirmOpen({
      ...
      message: "¿Quieres que la IA busque y genere el plan de mantenimiento oficial para este vehículo?",
      onConfirm: () => {
        startTransition(async () => {
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {isPending ? "Analizando..." : isPro ? "Autodescubrir Plan" : "Autodescubrir PRO"}
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
            <thead>
              <tr>
                ...
                {columns.map(km => (
                  <th key={km} className="p-3 border-b border-zinc-200 dark:border-zinc-700 text-center text-zinc-700 dark:text-zinc-300 font-bold min-w-[60px] transition-colors">
                    {km / 1000}
                  </th>
                ))}
              </tr>
              ...
              {tasks.filter(t => t.frequencyKm).map(task => (
                <tr key={task.id} className="border-b border-zinc-200 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors group">
                  ...
                  {columns.map(km => {
                    ...
                    if (!isRequired) {
                      return <td key={km} className="p-3 text-center text-zinc-300 dark:text-zinc-800 border-l border-zinc-200 dark:border-zinc-800/30 transition-colors"></td>;
                    }
