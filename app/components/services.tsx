    ...
    <div className="view-shell text-zinc-100 space-y-8 p-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-2 text-sm font-medium hover:bg-zinc-800 transition-colors">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950/40 p-6 flex flex-col justify-between h-32">
          ...
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950/40 p-6 flex flex-col justify-between h-32">
            ...
            <p className="text-3xl font-bold text-white">${totalSpendThisYear.toLocaleString()}</p>
          </div>
        </div>

        {/* Category Breakdown Card */}
        <div className="lg:col-span-2 rounded-[32px] border border-zinc-800 bg-zinc-950/40 p-8 backdrop-blur-md">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-white">Análisis por Categoría</h3>
            <span className="text-xs text-zinc-500">Distribución de gastos</span>
          </div>
          <div className="space-y-4">
            {expenseBreakdown.length > 0 ? expenseBreakdown.map(cat => (
              ...
              <div className="flex h-24 items-center justify-center text-zinc-600 italic text-sm">
      ...
      <div className="space-y-4">
        {currentItems.length > 0 ? (
          currentItems.map((item) => (
            <div key={item.id} className="rounded-3xl border border-zinc-800 bg-zinc-950/40 p-6 hover:bg-zinc-900/50 transition-all group">
              <div className="grid gap-6 md:grid-cols-5 items-center">
                ...
                <div className="space-y-1 md:col-span-1">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <p className="font-semibold text-zinc-200 line-clamp-1">{item.customName || "Mantenimiento General"}</p>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-bold text-white">{item.kmAtService.toLocaleString()} km</p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3">
                    <p className="font-bold text-white">${item.cost?.toLocaleString() || "0.00"}</p>
                ...
                <div className="flex justify-end items-center gap-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-800 text-zinc-400 hover:text-white transition-colors cursor-pointer">
                  ...
                  <button 
                    ...
                    className="rounded-xl border border-zinc-700 px-6 py-2 text-sm font-bold text-white hover:bg-zinc-800 transition-all"
                  >
                    Detalles
                  </button>
                </div>
      ...
      {selectedService && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="relative w-full max-w-2xl rounded-[32px] border border-zinc-800 bg-zinc-950 p-8 shadow-2xl animate-in fade-in zoom-in duration-200">
            <button 
              onClick={() => setSelectedService(null)}
              className="absolute right-6 top-6 flex h-10 w-10 items-center justify-center rounded-full border border-zinc-800 text-zinc-400 hover:bg-zinc-900 hover:text-white transition-colors"
            >
              ✕
            </button>
            ...
            <div className="space-y-8">
              <div>
                <h2 className="text-3xl font-bold text-white tracking-tight">Detalles del Servicio</h2>
                <p className="mt-1 text-zinc-400">{selectedService.customName || "Mantenimiento General"}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-semibold text-zinc-200">
                    {new Date(selectedService.date).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">KILOMETRAJE</p>
                  <p className="text-sm font-semibold text-zinc-200">{selectedService.kmAtService.toLocaleString()} km</p>
                </div>
                <div className="space-y-1">
              ...
              {selectedService.imageUrl && (
                <div className="space-y-3">
                  ...
                  <div className="relative aspect-video w-full rounded-2xl border border-zinc-800 bg-zinc-900 overflow-hidden group">
                    <img 
              ...
              <button 
                onClick={() => setSelectedService(null)}
                className="w-full rounded-2xl bg-zinc-900 py-3 text-sm font-bold text-white hover:bg-zinc-800 transition-all border border-zinc-800"
              >
                Cerrar Detalles
              </button>
            </div>
